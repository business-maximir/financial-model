...
const ModelDetails=(()=>{
  ...
  const royaltyScale=[
    {threshold:'up to 10,000,000 Toman', rate:'0%'},
    {threshold:'over 10,000,000 Toman', rate:'10%'},
    {threshold:'over 20,000,000 Toman', rate:'15%'},
    {threshold:'over 30,000,000 Toman', rate:'20%'},
    {threshold:'over 40,000,000 Toman', rate:'25%'},
    {threshold:'over 50,000,000 Toman', rate:'30%'},
    {threshold:'over 500,000,000 Toman', rate:'35%'},
    {threshold:'over 1,000,000,000 Toman', rate:'40%'}
  ];

  const startupPackage=[
    {image:'assets/gallery/package_euroflyer.jpg', titleRu:'Еврофлаер 200×100', titleEn:'Euro-flyer 200×100', titleFa:'تراکت دو رو ۲۰۰×۱۰۰', descRu:'Размещается на лобовых стеклах припаркованных автомобилей для привлечения водителей.', descEn:'Placed on windshields of parked cars to attract drivers.', descFa:'روی شیشه جلوی خودروهای پارک‌شده قرار می‌گیرد تا رانندگان جذب شوند.'},
  ...
  const copy={
    'ru-RU':{
      ...
      category:'Вид рекламы', annualShare:'Доля за год', unitCost:'Цена единицы, Toman', approxQty:'Ориентир, ед./год', total:'Итого',
    ...
    'en-US':{
      ...
      category:'Advertising type', annualShare:'Annual share', unitCost:'Unit cost, Toman', approxQty:'Approx. units/year', total:'Total',
      ...
      allAmounts:'All amounts are shown in Toman',
    ...
    'fa-IR':{
      ...
      category:'نوع تبلیغ',annualShare:'سهم سالانه',unitCost:'هزینه واحد، تومان',approxQty:'تعداد تقریبی در سال',total:'جمع',galleryTitle:'نمونه‌ها',
      royaltyRevenue:'درآمد مبنا',royaltyRate:'درصد رویالتی',cohortPopulation:'جمعیت شهر',cohortFee:'هزینه خرید حق امتیاز',feeTableTitle:'هزینه خرید حق امتیاز بر اساس گروه جمعیتی شهر',royaltyTableTitle:'جدول پلکانی رویالتی',
      assistantBase:'مبنای محاسبه',assistantFee:'هزینه خدمات همیار',assistantTableTitle:'تعرفه خدمات همیار',
      sectionInvestmentTitle:'سرمایه‌گذاری',sectionIncomeTitle:'درآمد',sectionExpensesTitle:'هزینه‌های عملیاتی',sectionProfitTitle:'سود',allAmounts:'تمام مبالغ به تومان',
  ...
  function subdetailBtn(key,label){const meta={startupPackage:{icon:'briefcase',ru:'Макеты и материалы, которые входят в стартовый пакет',en:'Layouts and materials included in the starter package',fa:'نمونه‌ها و اقلام موجود در بسته شروع'},advertisingPlan:{icon:'calendar',ru:'12-месячное распределение рекламного бюджета',en:'12-month budget allocation by advertising material',fa:'توزیع بودجه تبلیغات در ۱۲ ماه'},brandbook:{icon:'briefcase',ru:'Примеры рекламных материалов Maxim',en:'Examples of Maxim advertising materials',fa:'نمونه‌های واقعی تبلیغات ماکسیم'}}[key]||{icon:'info',ru:'',en:''};const sub=lang()==='fa-IR'?meta.fa:lang()==='en-US'?meta.en:meta.ru;return `<button class="detail-link-card" type="button" data-open-subdetail="${esc(key)}"><span class="detail-link-card__icon" aria-hidden="true">${iconSvg(meta.icon)}</span><span class="detail-link-card__content"><span class="detail-link-card__title">${esc(label)}</span><span class="detail-link-card__subtitle">${esc(sub)}</span></span><span class="detail-link-card__chevron" aria-hidden="true">›</span></button>`;}
  function feeTable(){
    const cc=c();
    const rows=lang()==='fa-IR'?[
      ['تا ۲۰٬۰۰۰ نفر','۴۰٬۰۰۰٬۰۰۰ تومان'],
      ['۲۰٬۰۰۱ تا ۵۰٬۰۰۰ نفر','۵۰٬۰۰۰٬۰۰۰ تومان'],
      ['۵۰٬۰۰۱ تا ۱۰۰٬۰۰۰ نفر','۶۰٬۰۰۰٬۰۰۰ تومان'],
      ['بیش از ۱۰۰٬۰۰۰ نفر','۷۰٬۰۰۰٬۰۰۰ تومان'],
      ['بوشهر، مراغه، سیرجان، بجنورد، زنجان','۱۰۰٬۰۰۰٬۰۰۰ تومان']
    ]:lang()==='en-US'?[
      ['Up to 20,000 people','40,000,000 Toman'],
      ['20,001–50,000 people','50,000,000 Toman'],
      ['50,001–100,000 people','60,000,000 Toman'],
      ['More than 100,000 people','70,000,000 Toman'],
      ['Bushehr, Maragheh, Sirjan, Bojnurd, Zanjan','100,000,000 Toman']
    ]:[
      ['До 20 000 человек','40 000 000 Toman'],
      ['20 001–50 000 человек','50 000 000 Toman'],
      ['50 001–100 000 человек','60 000 000 Toman'],
      ['Свыше 100 000 человек','70 000 000 Toman'],
      ['Bushehr, Maragheh, Sirjan, Bojnurd, Zanjan','100 000 000 Toman']
    ];
  ...
  function royaltyTable(){
    const cc=c();
    const rows=lang()==='fa-IR'?[
        ['تا ۱۰٬۰۰۰٬۰۰۰ تومان','۰٪'],['بیش از ۱۰٬۰۰۰٬۰۰۰ تومان','۱۰٪'],['بیش از ۲۰٬۰۰۰٬۰۰۰ تومان','۱۵٪'],['بیش از ۳۰٬۰۰۰٬۰۰۰ تومان','۲۰٪'],['بیش از ۴۰٬۰۰۰٬۰۰۰ تومان','۲۵٪'],['بیش از ۵۰٬۰۰۰٬۰۰۰ تومان','۳۰٪'],['بیش از ۵۰۰٬۰۰۰٬۰۰۰ تومان','۳۵٪'],['بیش از ۱٬۰۰۰٬۰۰۰٬۰۰۰ تومان','۴۰٪']
      ]:lang()==='en-US'?royaltyScale.map(r=>[esc(r.threshold),esc(r.rate)]):[
        ['До 10 000 000 Toman','0%'],
        ['Свыше 10 000 000 Toman','10%'],
        ['Свыше 20 000 000 Toman','15%'],
        ['Свыше 30 000 000 Toman','20%'],
        ['Свыше 40 000 000 Toman','25%'],
        ['Свыше 50 000 000 Toman','30%'],
        ['Свыше 500 000 000 Toman','35%'],
        ['Свыше 1 000 000 000 Toman','40%']
      ];
    return tableHtml(cc.royaltyTableTitle,[cc.royaltyRevenue,cc.royaltyRate],rows);
  }
  function assistantTable(){
    const cc=c();
    const rows=lang()==='fa-IR'?[['تا ۱۰٬۰۰۰٬۰۰۰ تومان','۱٬۵۰۰٬۰۰۰ تومان'],['بیش از ۱۰٬۰۰۰٬۰۰۰ تا ۵۰٬۰۰۰٬۰۰۰ تومان','۳٬۵۰۰٬۰۰۰ تومان'],['بیش از ۵۰٬۰۰۰٬۰۰۰ تومان','۷٬۵۰۰٬۰۰۰ تومان']]:lang()==='en-US'?[['Up to 10,000,000 Toman','1,500,000 Toman'],['Over 10,000,000 to 50,000,000 Toman','3,500,000 Toman'],['Over 50,000,000 Toman','7,500,000 Toman']]:[['До 10 000 000 Toman','1 500 000 Toman'],['Свыше 10 000 000 до 50 000 000 Toman','3 500 000 Toman'],['Свыше 50 000 000 Toman','7 500 000 Toman']];
    return tableHtml(cc.assistantTableTitle||tx('Тариф «Ассистент»','Assistant fee','تعرفه خدمات همیار'),[cc.assistantBase||tx('Расчётная база','Calculation base','مبنای محاسبه'),cc.assistantFee||tx('Стоимость','Fee','هزینه')],rows);
  }
  ...
  function definitionMap(model){
    const feeValue=fmtMoney(model?.summary?.initialPayment||model?.assumptions?.initialInvestment||0)+' '+(lang()==='fa-IR'?'تومان':'Toman');
    ...
    const definitions={
      investment:{title:tx('Инвестиции','Investment','سرمایه‌گذاری'),subtitle:c().allAmounts,html:`
        ${block(tx('Паушальный взнос','Franchise fee','هزینه خرید حق امتیاز'),tx(
          ...
          `هزینه خرید حق امتیاز یک پرداخت یک‌باره برای استفاده از مجوز، برند و فناوری‌های ماکسیم در شهر انتخاب‌شده است. مبلغ پایه برای این شهر <strong>${esc(feeValue)}</strong> است.`),'briefcase')}
  ...
  function openSubdetail(key,model){
    ...
    if(key==='startupPackage'){
      ...
      subtitle.textContent=isFa?'نمونه‌های واقعی تبلیغات ماکسیم در ایران.':isRu?'Реальные примеры размещения рекламы Maxim в Иране.':'Real examples of Maxim advertising placements in Iran.';
