...
const Charts=(()=>{
  ...
  function renderInlineLegend(){
    ...
    holder.innerHTML=chart.data.datasets.slice(0,2).map((dataset,index)=>{
      ...
      return `<button class="chart-legend-chip${visible?'':' is-muted'}" type="button" data-dataset-index="${index}" title="${Localization.t('toggleChartSeries',{label:dataset.label})}"><span class="chart-legend-dot" style="background:${color}"></span><span>${dataset.label}</span></button>`;
    }).join('');
  }
